...
class _FocusTimerScreenState extends ConsumerState<FocusTimerScreen>
  ...
  void initState() {
    super.initState();
    _starController = AnimationController(vsync: this, duration: const Duration(seconds: 60))
      ..repeat();
    _loadPersistedState();
  }
  ...
  Future<void> _persistState() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt(_keyStartTs, DateTime.now().millisecondsSinceEpoch ~/ 1000);
    await prefs.setInt(_keyDuration, _totalSeconds);
    if (_activeMissionId != null) prefs.setString(_keyMissionId, _activeMissionId!);
  }
  ...
  Future<void> _onComplete() async {
    ...
    await _clearPersistedState();
    setState(() { _running = false; _complete = true; });
    // Log to Supabase
    try {
      await Supabase.instance.client.from('focus_sessions').insert({
        'user_id': Supabase.instance.client.auth.currentUser!.id,
        'mission_id': _activeMissionId,
        'started_at': DateTime.now().subtract(Duration(seconds: _totalSeconds)).toIso8601String(),
        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              const Expanded(child: Text('Deep Space Mode', textAlign: TextAlign.center,
              ...
              Padding(
                ...
                child: OutlinedButton(
                  ...
                  style: OutlinedButton.styleFrom(
                    foregroundColor: _totalSeconds == m * 60 ? AppTheme.primary : Colors.grey,
                    side: BorderSide(color: _totalSeconds == m * 60 ? AppTheme.primary : Colors.grey),
                ...
                children: [
                  Text(
                    ...
                    style: const TextStyle(
                      fontSize: 56, fontWeight: FontWeight.bold, color: AppTheme.text,
                      letterSpacing: 4,
                    ),
                  ),
                  if (_activeMissionId != null)
                    const Text('Mission Active', style: TextStyle(color: AppTheme.accent, fontSize: 12)),
                ],
              ),
            ),
          ),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Text('Mission Complete!', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold)),
          const SizedBox(height: 12),
          const Text('Outstanding work, Commander.', style: TextStyle(color: Colors.grey)),
...
class _TimerRingPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawArc(Rect.fromCircle(center: center, radius: radius),
        -pi / 2, progress * 2 * pi, false, glowPaint);
...
class _StarfieldPainter extends CustomPainter {
  ...
  static final _stars = List.generate(120, (_) => Offset(_rand.nextDouble(), _rand.nextDouble()));
  static final _sizes = List.generate(120, (_) => _rand.nextDouble() * 2.5 + 0.5);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < _stars.length; i++) {
      ...
      paint.color = Colors.white.withOpacity(0.3 + 0.7 * twinkle);
      canvas.drawCircle(
