...
class GalaxyMapScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
            ...
            return const Center(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 16),
                  Text(
                    'No galaxies yet, Commander.',
                    style: TextStyle(color: Colors.grey),
                  ),
                  SizedBox(height: 8),
                  Text(
                    'Create a galaxy to start your map.',
      ...
      bottomNavigationBar: const SpaceyBottomNav(currentIndex: 3),
    );
  }
  ...
  void _showCreateGalaxyDialog(BuildContext context, WidgetRef ref) {
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: TextField(
          controller: ctrl,
          decoration: const InputDecoration(hintText: 'Galaxy name'),
        ),
        actions: [
          TextButton(
            ...
            onPressed: () async {
              if (ctrl.text.trim().isNotEmpty) {
                await ref
                    ...
                    .createGalaxy(ctrl.text.trim(), null);
...
class _GalaxyCanvasState extends ConsumerState<_GalaxyCanvas>
  ...
  void initState() {
    ...
    _galaxyPositions = List.generate(widget.galaxies.length, (i) {
      return Offset(
        200 + _rand.nextDouble() * 800,
        100 + _rand.nextDouble() * 700,
      );
    });
  }
...
class _GalaxyMapPainter extends CustomPainter {
  ...
  static final _starSizes = List.generate(
    200,
    (_) => _rand.nextDouble() * 1.5 + 0.5,
  );
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < galaxies.length; i++) {
      ...
      for (int j = 0; j < 8; j++) {
        ...
        final r = 6.0 + j * 0.5;
        final sp = Offset(
          pos.dx + cos(angle) * r * 0.6,
          pos.dy + sin(angle) * r * 0.6,
        );
        canvas.drawCircle(
          sp,
          1.5,
